...
def create_node_features(graph, weather_data):
    """Creates a scaled feature tensor for GNN input with 7 features."""
    ...
    for node in node_list:
        ...
        # Calculate topology features (REQUIRED for model compatibility)
        degree = graph.degree(node)
        neighbors = list(graph.neighbors(node))
        if neighbors:
            avg_neighbor_distance = np.mean([
                utils.calculate_haversine_distance(node[0], node[1], n[0], n[1]) 
                for n in neighbors
            ])
        else:
            avg_neighbor_distance = 0
        
        # Feature vector: [lat, lon, wave_height, wave_period, ocean_current_velocity, degree, avg_neighbor_distance]
        features = [
            node[0], 
            node[1], 
            weather['wave_height'], 
            weather['wave_period'], 
            weather['ocean_current_velocity'],
            degree,
